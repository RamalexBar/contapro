...
| **Facturación electrónica DIAN (iteración 4-6, sin UI web todavía)** | Generación local de CUFE/CUDE (SHA-384) y XML tipo UBL 2.1 por cada venta completada **y por cada nota crédito/débito** que referencia una venta ya facturada, con resolución de numeración DIAN real (prefijo/rango/consecutivo, atómico, por tipo de documento) separada del consecutivo interno del POS. Ventas sin cliente usan un identificador genérico de "consumidor final" (constante aislada, pendiente de verificar contra el Anexo Técnico). Firma XAdES-BES (certificado PKCS#12, probada con certificado autofirmado) y envío asíncrono a la DIAN (un poller en proceso por tipo de documento) implementados pero **no verificados contra el servicio real de la DIAN** — faltan credenciales de habilitación para probarlos end-to-end. Ver `apps/api/src/modules/electronic-invoicing/README.md` para la lista completa de detalles sin verificar (XAdES-EPES vs BES, formato exacto del SOAP, códigos de tipo de nota, límite de una sola instancia del poller) |
