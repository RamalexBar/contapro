...
| **Facturación electrónica DIAN (iteración 4-7, sin UI web todavía)** | Generación local de CUFE/CUDE/CUDS (SHA-384) y XML tipo UBL por cada venta completada, cada nota crédito/débito que referencia una venta ya facturada, **y cada compra a un proveedor marcado como no obligado a facturar electrónicamente** (documento soporte), con resolución de numeración DIAN real (prefijo/rango/consecutivo, atómico, por tipo de documento) separada del consecutivo interno del POS. Ventas sin cliente usan un identificador genérico de "consumidor final" (constante aislada, pendiente de verificar contra el Anexo Técnico). Firma XAdES-BES (certificado PKCS#12, probada con certificado autofirmado) y envío asíncrono a la DIAN (un poller en proceso por tipo de documento) implementados pero **no verificados contra el servicio real de la DIAN** — faltan credenciales de habilitación para probarlos end-to-end. El documento soporte es la parte menos verificada de todo el módulo (su esquema XML tiene mucha menos documentación pública que factura/notas). Ver `apps/api/src/modules/electronic-invoicing/README.md` para la lista completa de detalles sin verificar |
...
8. ~~Facturación electrónica: firma XAdES + envío asíncrono a la DIAN~~ — implementado en la
   ...
   credenciales de habilitación reales para probar end-to-end, ver iteración 4-5 arriba).
