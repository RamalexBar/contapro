...
| **Facturación electrónica DIAN (mínimo, iteración 4, sin UI web todavía)** | Generación local de CUFE (SHA-384) y XML tipo UBL 2.1 por cada venta completada, con resolución de numeración DIAN real (prefijo/rango/consecutivo, atómico) separada del consecutivo interno del POS. Ventas sin cliente usan un identificador genérico de "consumidor final" (constante aislada, pendiente de verificar contra el Anexo Técnico). Firma XAdES y envío real a la DIAN quedan pendientes de credenciales de habilitación — ver `apps/api/src/modules/electronic-invoicing/README.md` |
...
8. Facturación electrónica: firma XAdES + envío real a la DIAN una vez existan credenciales de
   habilitación (la generación local de CUFE/XML ya es funcional, ver iteración 4).
