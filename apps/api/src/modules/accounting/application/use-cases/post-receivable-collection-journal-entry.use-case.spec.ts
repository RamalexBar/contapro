...
import { PostReceivableCollectionJournalEntryUseCase } from "./post-receivable-collection-journal-entry.use-case";
...
function makeUseCase() {
  ...
  return { useCase: new PostReceivableCollectionJournalEntryUseCase(accountRepo, createEntry, postEntry), journalRepo };
}

describe("PostReceivableCollectionJournalEntryUseCase", () => {
  it("does not post anything when the collected amount is zero", async () => {
    const { useCase, journalRepo } = makeUseCase();

    const entry = await withTenantContext(() =>
      useCase.execute({
        accountReceivablePaymentId: "arp-1",
        branchId: "branch-1",
  ...
  it("debits Caja and credits Clientes for a CASH collection", async () => {
    const { useCase, journalRepo } = makeUseCase();
    ...
    expect(lines.find((l) => l.accountId === "acc-1305")).toMatchObject({ debit: 0, credit: 50_000 });
    expect(lines.find((l) => l.accountId === "acc-1110")).toBeUndefined();
  });

  it("debits Bancos (not Caja) for a WOMPI collection", async () => {
    const { useCase, journalRepo } = makeUseCase();

    await withTenantContext(() =>
      useCase.execute({
        accountReceivablePaymentId: "arp-2",
        branchId: "branch-1",
        date: new Date("2026-08-05"),
        customerName: "Juan Perez",
        amount: 120_000,
    ...
    const lines = journalRepo.entries[0].lines;
    expect(lines.find((l) => l.accountId === "acc-1110")).toMatchObject({ debit: 120_000, credit: 0 });
    expect(lines.find((l) => l.accountId === "acc-1305")).toMatchObject({ debit: 0, credit: 120_000 });
