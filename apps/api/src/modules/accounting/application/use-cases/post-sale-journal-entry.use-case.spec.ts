...
import { PostSaleJournalEntryUseCase } from "./post-sale-journal-entry.use-case";
...
function makeUseCase() {
  ...
  return { useCase: new PostSaleJournalEntryUseCase(accountRepo, createEntry, postEntry), journalRepo };
}

const NO_WITHHOLDINGS = { RETEFUENTE: 0, RETEICA: 0, RETEIVA: 0 } as const;

describe("PostSaleJournalEntryUseCase", () => {
  it("does not post anything when the sale total is zero", async () => {
  ...
  it("balances a sale without any retention (baseline, unaffected by the new fields)", async () => {
    const { useCase, journalRepo } = makeUseCase();
    ...
    expect(lines.some((l) => l.accountId.startsWith("acc-1355"))).toBe(false);
    const totalDebit = lines.reduce((sum, l) => sum + l.debit, 0);
    const totalCredit = lines.reduce((sum, l) => sum + l.credit, 0);
    expect(totalDebit).toBe(totalCredit);
  });

  it("debits the anticipo account for a single retention type and reduces cash received by the retained amount", async () => {
    const { useCase, journalRepo } = makeUseCase();
    ...
    await withTenantContext(() =>
      useCase.execute({
        ...
        withholdingsByType: { ...NO_WITHHOLDINGS, RETEFUENTE: 2_500 },
        // netTotal = 119000 - 2500 = 116500, pagado en efectivo
        payments: [{ method: "CASH", amount: 116_500 }],
      })
    );

    const lines = journalRepo.entries[0].lines;
    expect(lines.find((l) => l.accountId === "acc-1105")).toMatchObject({ debit: 116_500, credit: 0 });
    expect(lines.find((l) => l.accountId === "acc-135515")).toMatchObject({ debit: 2_500, credit: 0 });
    expect(lines.find((l) => l.accountId === "acc-4135")).toMatchObject({ debit: 0, credit: 100_000 });
    expect(lines.find((l) => l.accountId === "acc-2408")).toMatchObject({ debit: 0, credit: 19_000 });
    const totalDebit = lines.reduce((sum, l) => sum + l.debit, 0);
    const totalCredit = lines.reduce((sum, l) => sum + l.credit, 0);
    expect(totalDebit).toBe(totalCredit);
    expect(totalDebit).toBe(119_000);
  });

  it("only creates anticipo lines for retention types with a non-zero amount, one account per type", async () => {
    const { useCase, journalRepo } = makeUseCase();
    ...
    await withTenantContext(() =>
      useCase.execute({
        ...
        taxTotal: 19_000,
        total: 119_000,
        retentionTotal: 5_350,
        withholdingsByType: { RETEFUENTE: 2_500, RETEICA: 0, RETEIVA: 2_850 },
        payments: [{ method: "CASH", amount: 113_650 }],
      })
    );

    const lines = journalRepo.entries[0].lines;
    expect(lines.find((l) => l.accountId === "acc-135515")).toMatchObject({ debit: 2_500, credit: 0 });
    expect(lines.find((l) => l.accountId === "acc-135517")).toBeUndefined();
  ...
  it("still generates receivable (Clientes) on top of a retained credit sale", async () => {
    ...
    await withTenantContext(() =>
      useCase.execute({
        ...
        retentionTotal: 2_500,
        withholdingsByType: { ...NO_WITHHOLDINGS, RETEFUENTE: 2_500 },
        payments: [], // credito puro: nada cobrado todavia
      })
    );

    const lines = journalRepo.entries[0].lines;
    // netTotal (116500) queda todo como cuenta por cobrar, mas el anticipo de 2500
    expect(lines.find((l) => l.accountId === "acc-1305")).toMatchObject({ debit: 116_500, credit: 0 });
    expect(lines.find((l) => l.accountId === "acc-135515")).toMatchObject({ debit: 2_500, credit: 0 });
    const totalDebit = lines.reduce((sum, l) => sum + l.debit, 0);
    expect(totalDebit).toBe(119_000);
  });
...
describe("PostSaleJournalEntryUseCase — multi-moneda informativa (item 33 de docs/ALCANCE.md)", () => {
  it("keeps the plain description when currency is omitted or COP", async () => {
    ...
    expect(journalRepo.entries[0].description).toBe("Venta #45");
  });
  ...
  it("annotates the description with the foreign currency and TRM, leaving debit/credit amounts in COP", async () => {
    ...
    expect(entry.description).toBe("Venta #45 (USD 28.33 @ TRM 4200)");
